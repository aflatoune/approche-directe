...
At December 2018, all values in `fr_x` are observed except Banque de France surveys in December, household consumption in December and IPI in November and December. Thus, the columns containing these values must be truncated and extended with an ARIMA model each time a forecast is performed:

```{r}
bdf_3_cols <- names(fr_x)[stringr::str_detect(names(fr_x), "Bdf_3")]
...
extend_cols <- c(bdf_3_cols, conso3_cols, ipi2_3_cols)
n_extend <- c(rep(1, 8), rep(2, 4))
list_cols <- list(extend_cols, n_extend)
# forecast 1 quarter ahead for BdF and consumption series (i.e. for 8 series)
# and 2 quarters ahed for IPI series (i.e. for 4 series)
```
